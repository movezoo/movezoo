...
const Main = (props) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      let playerSegment = findSegment(position + playerZ);
      let playerPercent = Util.percentRemaining(
        position + playerZ,
        segmentLength
      );
      let playerY = Util.interpolate(
      ...
      for (let n = 0; n < drawDistance; n++) {
        ...
        if (
          segment.p1.camera.z <= cameraDepth || // 화면 뒤에 있는 경우
          segment.p2.screen.y >= segment.p1.screen.y || // 뒷면 가림
          segment.p2.screen.y >= maxy
        )
          // 이미 렌더링된 언덕에 의해 클립되는 경우
          continue;
    ...
    <div>
      <table id="controls" style={{ display: "none" }}>
        <tbody>
          ...
          <tr>
            <th>
              <label htmlFor="roadWidth">
                Road Width (<span id="currentRoadWidth"></span>) :
              </label>
            </th>
            <td>
              <input
          ...
          <tr>
            <th>
              <label htmlFor="cameraHeight">
                CameraHeight (<span id="currentCameraHeight"></span>) :
              </label>
            </th>
            <td>
              <input
          ...
          <tr>
            <th>
              <label htmlFor="drawDistance">
                Draw Distance (<span id="currentDrawDistance"></span>) :
              </label>
            </th>
            <td>
              <input
          ...
          <tr>
            <th>
              <label htmlFor="fieldOfView">
                Field of View (<span id="currentFieldOfView"></span>) :
              </label>
            </th>
            <td>
              <input
          ...
          <tr>
            <th>
              <label htmlFor="fogDensity">
                Fog Density (<span id="currentFogDensity"></span>) :
              </label>
            </th>
            <td>
              <input
      ...
      <div id="instructions" style={{ display: "none" }}>
        <p>
          Use the <b>arrow keys</b> to drive the car.
        </p>
        내차 X 위치 : <span id="playerX">0</span>
        <br />
        내차 Z 위치 : <span id="position">0</span>
        <br />
        상대차 X 위치 : <span id="userX">0</span>
        <br />
        상대차 Z 위치 : <span id="userZ">0</span>
        <br />
        trackLength : <span id="trackLength">0</span>
        <br />
      </div>
      ...
      <div id="racer">
        <div style={{ display: "none" }}>
          <div id="hud">
            <span id="speed" className="hud">
              <span id="speed_value" className="value">
                0
              </span>{" "}
              mph
            </span>
            <span id="current_lap_time" className="hud">
              Time:{" "}
              <span id="current_lap_time_value" className="value">
                0.0
              </span>
            </span>
            <span id="last_lap_time" className="hud">
              Last Lap:{" "}
              <span id="last_lap_time_value" className="value">
                0.0
              </span>
            </span>
            <span id="fast_lap_time" className="hud">
              Fastest Lap:{" "}
              <span id="fast_lap_time_value" className="value"></span>
            </span>
          </div>
          {/* <canvas id="canvas">
